#!/bin/bash
# Pre-Release Sanitizer for CtxRtr
...
PERSONAL_NAME="CtxRtr Maintainer"
...
print_header() {
    echo -e "${BLUE}================================${NC}"
    echo -e "${BLUE}  CtxRtr Pre-Release Sanitizer${NC}"
    echo -e "${BLUE}================================${NC}"
    echo
}
    ...
    for file in "${files_to_fix[@]}"; do
        if [[ -f "$PROJECT_ROOT/$file" ]]; then
            if [[ "$DRY_RUN" == true ]]; then
                ...
                sed -i '' 's|/Users/[^/]*/Dropbox/Admin/CtxRtr/|./|g' "$PROJECT_ROOT/$file"
...
---
*Generated by CtxRtr Pre-Release Sanitizer*
EOF
